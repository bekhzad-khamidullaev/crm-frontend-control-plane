# AGENTS.md — Frontend на основе `React + Ant Design 5.x` + подключение к API по схеме `enterprise-crm-api.yaml`

Коротко, по делу. Документ описывает автономных агентов Codex CLI (или CI-агентов) и рабочий процесс для разработки фронтенда CRM на базе `React 18 + Ant Design 5.x` с интеграцией по API-схеме `enterprise-crm-api.yaml`. Принцип: модульная разработка → начать с `leads` и довести до 100% функционала, затем следующий модуль. Код — senior-level: читаемый, тестируемый, масштабируемый.
...
- API-схема: `enterprise-crm-api.yaml` (источник правды для всех запросов)
...
│  ├─ lib/
│  │  └─ api/                   # wrapper для запросов по enterprise-crm-api.yaml
│  │     └─ client.js
...
- [ ] Преобразовать `enterprise-crm-api.yaml` в понятные вызовы (client wrapper).
...
- Источник правды — `enterprise-crm-api.yaml`. Любые изменения API — фиксируются в YAML и синхронизируются в client-wrapper.
...
- Если backend меняет API — обновить `enterprise-crm-api.yaml` и синхронизировать в `client.js`
