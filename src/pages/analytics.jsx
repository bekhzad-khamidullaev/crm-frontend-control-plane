...
import { AnalyticsCard, AnimatedChart, PredictionChart, PredictionModelStatus } from '../components/analytics';
import { formatCurrencyForRecord, formatNumber, resolveCurrencyCode } from '../lib/utils/format.js';
import {
  formatAdditiveSummaryValue,
  formatAnalyticsMonetaryValue,
  formatUnitSummaryValue,
} from '../lib/utils/analyticsCurrency.js';
...
const FORECAST_HORIZON_OPTIONS = [7, 14, 30, 60, 90];
...
function buildPredictionPayload(data, fallbackTitle) {
  ...
  const currencyCode = resolveCurrencyCode(data, {
    currencyKeys: ['currency_code', 'currency_name', 'currency', 'summary_currency', 'revenue_currency_code'],
    fallback: null,
  });
  const forecastItemCurrencyCode = Array.isArray(data?.forecasts)
    ? resolveCurrencyCode(data.forecasts[0], {
        currencyKeys: ['currency_code', 'currency_name', 'currency', 'summary_currency', 'revenue_currency_code'],
        fallback: null,
      })
    : null;
  const resolvedCurrencyCode = currencyCode || forecastItemCurrencyCode;
...
function buildOverviewRevenueHint(overview, tr) {
  if (!overview || typeof overview !== 'object') return null;

  const amountCurrencyCodes = Array.isArray(overview.amount_currency_codes)
    ? overview.amount_currency_codes.filter((code) => typeof code === 'string' && code.trim())
    : [];

  if (overview.amount_display_mode === 'mixed_currency_unscaled') {
    const currenciesLabel = amountCurrencyCodes.length ? amountCurrencyCodes.join(', ') : tr('analyticsPage.common.multipleCurrencies', 'multiple currencies');
    return tr(
      'analyticsPage.summary.mixedRevenueHint',
      'Revenue combines multiple currencies and is shown as raw totals ({{currencies}}).',
      { currencies: currenciesLabel }
    );
  }

  if (overview.amount_display_mode === 'single_currency' && overview.amount_currency) {
    return tr(
      'analyticsPage.summary.singleCurrencyRevenueHint',
      'Revenue is shown in {{currency}}.',
      { currency: overview.amount_currency }
    );
  }
...
function buildMoneyChartOptions(currencyCode) {
  return {
    plugins: {
      tooltip: {
        callbacks: {
...
function AnalyticsBiReport({ rows = [], columns, emptyDescription, kpiConfig = [], rowKey = 'key' }) {
  if (!rows.length) {
  ...
  return (
    <Space direction="vertical" size="middle" style={{ width: '100%' }}>
      {kpiConfig.length ? (
        <Row gutter={[16, 16]}>
          {kpiConfig.map((item) => (
            <Col xs={24} sm={12} lg={kpiConfig.length > 3 ? 6 : 8} key={item.title}>
              <Statistic
                title={item.title}
                value={item.value}
...
export default function AnalyticsPage() {
  ...
  const [leadChannels, setLeadChannels] = useState(null);
  ...
  const [loadingLeadChannels, setLoadingLeadChannels] = useState(false);
  const [loadingMarketingCampaigns, setLoadingMarketingCampaigns] = useState(false);
  ...
  const [leadChannelsError, setLeadChannelsError] = useState(null);
  const [marketingCampaignsError, setMarketingCampaignsError] = useState(null);
  ...
  const loadCore = async () => {
    ...
    setLeadChannelsError(null);
    setMarketingCampaignsError(null);
    setLoadingOverview(true);
    setLoadingAnalytics(true);
    setLoadingFunnel(true);
    setLoadingActivity(true);
    setLoadingLeadChannels(true);
    setLoadingMarketingCampaigns(true);
    ...
    await Promise.allSettled([
      ...
      (async () => {
        try {
          setLeadChannels(await getLeadChannels({ period }));
        } catch (error) {
          setLeadChannelsError(error);
        } finally {
          setLoadingLeadChannels(false);
        }
      })(),
      (async () => {
        try {
          setMarketingCampaigns(await getMarketingCampaigns({ period }));
        } catch (error) {
          setMarketingCampaignsError(error);
        } finally {
          setLoadingMarketingCampaigns(false);
        }
      })(),
    ]);
  };
  ...
  const loadPredictions = async () => {
    ...
    await Promise.allSettled([
      ...
      (async () => {
        try {
          setRevenueForecast(await predictions.revenue.forecast({ days: forecastHorizon }));
        } catch (error) {
          setPredictionError(error);
        }
      })(),
      (async () => {
        try {
          setLeadsForecast(await predictions.leads.forecast({ days: forecastHorizon }));
        } catch (error) {
          setPredictionError(error);
        }
      })(),
      (async () => {
        try {
          setClientsForecast(await predictions.clients.forecast({ days: forecastHorizon }));
        } catch (error) {
          setPredictionError(error);
        }
      })(),
      (async () => {
        try {
          ...
          setNextActionsClients(Array.isArray(data) ? data : data?.predictions || data?.results || []);
        } catch (error) {
          setPredictionError(error);
        }
      })(),
      (async () => {
        try {
          ...
          setNextActionsDeals(Array.isArray(data) ? data : data?.predictions || data?.results || []);
        } catch (error) {
          setPredictionError(error);
        }
      })(),
  ...
  const handleRunPredictions = async () => {
    setLoadingPredictions(true);
    try {
      await predictions.predictAll({ horizon_days: forecastHorizon });
  ...
  const monthlyGrowthCurrencyCode = resolveCurrencyCode(monthlyGrowth, {
  ...
  const revenueChartOptions = useMemo(
    () => buildMoneyChartOptions(monthlyGrowthCurrencyCode),
    [monthlyGrowthCurrencyCode]
  );
  ...
  const modelRuns = useMemo(
    () => (Array.isArray(predictionStatus?.model_runs) ? predictionStatus.model_runs : []),
    [predictionStatus]
  );
  ...
  const leadChannelsRows = Array.isArray(leadChannels?.rows) ? leadChannels.rows : [];
  const leadChannelsSummary = leadChannels?.summary || {};
  const marketingCampaignRows = Array.isArray(marketingCampaigns?.rows) ? marketingCampaigns.rows : [];
  const marketingCampaignSummary = marketingCampaigns?.summary || {};
  const overviewRevenueDisplay = useMemo(
    () =>
      formatAdditiveSummaryValue(overview?.total_revenue ?? 0, {
        summary: overview,
        breakdownKeys: ['total_revenue_by_currency', 'revenue_by_currency'],
      }),
    [overview]
  );
  const overviewRevenueHint = useMemo(() => buildOverviewRevenueHint(overview, tr), [overview, tr]);
  const marketingCostDisplay = useMemo(
    () =>
      formatAdditiveSummaryValue(marketingCampaignSummary.total_cost ?? 0, {
        rows: marketingCampaignRows,
        amountKey: 'cost',
        summary: marketingCampaignSummary,
        breakdownKeys: ['total_cost_by_currency'],
      }),
    [marketingCampaignRows, marketingCampaignSummary]
  );
  const marketingRevenueDisplay = useMemo(
    () =>
      formatAdditiveSummaryValue(marketingCampaignSummary.total_revenue ?? 0, {
        rows: marketingCampaignRows,
        amountKey: 'revenue',
        summary: marketingCampaignSummary,
  ...
  const marketingCplDisplay = useMemo(
    () =>
      formatUnitSummaryValue(marketingCampaignSummary.cpl ?? 0, {
        rows: marketingCampaignRows,
        amountKey: 'cpl',
        summary: marketingCampaignSummary,
        mixedLabel: tr('analyticsPage.common.mixedCurrencies', 'Mixed currencies'),
      }),
    [marketingCampaignRows, marketingCampaignSummary, tr]
  );
  ...
  const leadChannelColumns = useMemo(
    () => [
      {
        title: tr('analyticsPage.table.channel', 'Channel'),
        dataIndex: 'channel',
        key: 'channel',
        render: (value) => <Text strong>{value || tr('analyticsPage.common.unknown', 'Unknown')}</Text>,
      },
      {
        title: tr('analyticsPage.metrics.leads', 'Leads'),
        dataIndex: 'leads',
        key: 'leads',
      },
      {
        title: tr('analyticsPage.metrics.deals', 'Deals'),
        dataIndex: 'deals',
        key: 'deals',
      },
      {
        title: tr('analyticsPage.metrics.conversion', 'Conversion'),
      ...
      },
      {
        ...
        dataIndex: 'revenue',
        key: 'revenue',
        render: (value, record) => formatCurrencyForRecord(value, record),
      },
    ],
    [tr]
  );
  ...
  const campaignColumns = useMemo(
    () => [
      {
        title: tr('analyticsPage.table.campaign', 'Campaign'),
        dataIndex: 'campaign',
        key: 'campaign',
        render: (value) => <Text strong>{value || tr('analyticsPage.common.unknown', 'Unknown')}</Text>,
      },
      {
        title: tr('analyticsPage.metrics.cost', 'Cost'),
        dataIndex: 'cost',
        key: 'cost',
        render: (value, record) => formatCurrencyForRecord(value, record),
      },
      {
        title: tr('analyticsPage.metrics.cpl', 'CPL'),
        dataIndex: 'cpl',
        key: 'cpl',
        render: (value, record) => formatCurrencyForRecord(value, record),
      },
      {
        title: tr('analyticsPage.metrics.dealConversion', 'Deal conversion'),
        dataIndex: 'conversion_rate',
        key: 'conversion_rate',
        render: (value) => formatPercentValue(value),
      },
  ...
  const tabs = [
    {
      ...
      children: (
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
                ...
                <Row gutter={[16, 16]}>
                  {focusCards.map((card) => (
                    <Col xs={24} md={8} key={card.key}>
                      <DecisionCard title={card.title} value={card.value} description={card.description} />
                    </Col>
                  ))}
                </Row>
              </Col>
              <Col xs={24} lg={8}>
                <AnalyticsCard title={tr('analyticsPage.sections.risks', 'Risks')} loading={loadingAnalytics || loadingOverview} error={analyticsError || overviewError} onRetry={handleRefresh}>
                  <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                    {riskCards.map((risk) => (
                      <RiskRow
                        key={risk.key}
          ...
          >
            <AnalyticsCard title={tr('analyticsPage.sections.keyMetrics', 'Key metrics')} loading={loadingOverview} error={overviewError} onRetry={loadCore}>
              <Row gutter={[16, 16]}>
                ...
                <Col xs={24} sm={12} lg={6}>
                  <Statistic
                    ...
                    formatter={() => overviewRevenueDisplay || formatNumber(overview?.total_revenue || 0)}
                    suffix={renderGrowthTag(overview?.revenue_growth)}
                  />
                </Col>
              </Row>
              <Space direction="vertical" size={4}>
                {overviewRevenueHint ? <Text type="secondary">{overviewRevenueHint}</Text> : null}
          ...
          <AnalyticsSection
            title={tr('analyticsPage.sections.biAnalytics', 'BI analytics')}
            description={tr('analyticsPage.sections.biAnalyticsDescription', 'Lead channels and marketing campaign performance for the selected period.')}
          >
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={12}>
                <AnalyticsCard
                  title={tr('analyticsPage.sections.leadChannels', 'Leads by channel')}
                  loading={loadingLeadChannels}
                  error={leadChannelsError}
                  onRetry={loadCore}
                >
                  <AnalyticsBiReport
                    rows={leadChannelsRows}
                    summary={leadChannelsSummary}
                    columns={leadChannelColumns}
                    emptyDescription={tr('analyticsPage.empty.noLeadChannelsData', 'No lead channels data')}
                    kpiConfig={[
                      {
                        title: tr('analyticsPage.metrics.totalLeads', 'Total leads'),
                        value: leadChannelsSummary.total_leads ?? 0,
                      },
                      {
                        title: tr('analyticsPage.metrics.totalDeals', 'Total deals'),
                        value: leadChannelsSummary.total_deals ?? 0,
                      },
                      {
                        title: tr('analyticsPage.metrics.conversion', 'Conversion'),
                        value: leadChannelsSummary.conversion_rate ?? 0,
              ...
              <Col xs={24} lg={12}>
                <AnalyticsCard
                  title={tr('analyticsPage.sections.marketingCampaigns', 'Marketing campaigns')}
                  loading={loadingMarketingCampaigns}
                  error={marketingCampaignsError}
                  onRetry={loadCore}
                >
                  <AnalyticsBiReport
                    rows={marketingCampaignRows}
                    ...
                    emptyDescription={tr('analyticsPage.empty.noCampaignsData', 'No marketing campaigns data')}
                    kpiConfig={[
                      {
                        title: tr('analyticsPage.metrics.cost', 'Cost'),
                        value: marketingCampaignSummary.total_cost ?? 0,
                        formatter: () => marketingCostDisplay,
                      },
                      {
                        title: tr('analyticsPage.metrics.cpl', 'CPL'),
                        value: marketingCampaignSummary.cpl ?? 0,
                        formatter: () => marketingCplDisplay,
                      },
                      {
                        title: tr('analyticsPage.metrics.dealConversion', 'Deal conversion'),
                        value: marketingCampaignSummary.conversion_rate ?? 0,
                        formatter: formatPercentValue,
                      },
                      {
                        title: tr('analyticsPage.metrics.revenue', 'Revenue'),
                        value: marketingCampaignSummary.total_revenue ?? 0,
                        formatter: () => marketingRevenueDisplay,
                      },
                    ]}
                    rowKey="key"
                  />
          ...
          >
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.charts.revenueDynamics', 'Revenue dynamics')} loading={loadingAnalytics} error={analyticsError} onRetry={loadCore}>
                  {revenueChartData ? (
                    <AnimatedChart type="line" data={revenueChartData} options={revenueChartOptions} height={280} />
                  ) : (
              ...
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.charts.leadsAndDeals', 'Leads and deals')} loading={loadingAnalytics} error={analyticsError} onRetry={loadCore}>
                  {leadsDealsChartData ? (
          ...
          >
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.charts.salesFunnel', 'Sales funnel')} loading={loadingFunnel} error={funnelError} onRetry={loadCore}>
                  {funnelChartData ? (
              ...
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.sections.taskStatuses', 'Task statuses')} loading={loadingAnalytics} error={analyticsError} onRetry={loadCore}>
                  <Row gutter={[16, 16]}>
                    <Col span={12}>
      ...
      children: (
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          ...
          >
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={16}>
                <AnalyticsCard title={tr('analyticsPage.sections.activityFeed', 'Activity feed')} loading={loadingActivity} error={activityError} onRetry={loadCore}>
              ...
              <Col xs={24} lg={8}>
                <AnalyticsCard title={tr('analyticsPage.sections.authSnapshot', 'Auth snapshot')} loading={loadingAuthStats} error={authError} onRetry={loadAuthStats}>
                  {authStats ? (
        ...
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          <Space>
            ...
            <Segmented
              ...
              options={FORECAST_HORIZON_OPTIONS.map((day) => ({ label: `${day}d`, value: day }))}
          ...
          >
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.predictions.revenueForecast', 'Revenue forecast')} loading={loadingPredictions} error={predictionError} onRetry={loadPredictions}>
                  {revenuePrediction ? (
                    revenuePrediction.type === 'interval' ? (
                      <PredictionChart
                        ...
                        currencyCode={revenuePrediction.currencyCode}
                        height={280}
                      />
                    ) : (
                      <AnimatedChart
                        ...
                        options={buildMoneyChartOptions(revenuePrediction.currencyCode)}
                        height={280}
                      />
                    )
                  ) : (
              ...
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.predictions.leadsForecast', 'Leads forecast')} loading={loadingPredictions} error={predictionError} onRetry={loadPredictions}>
                  {leadsPrediction ? (
          ...
          >
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.predictions.clientsForecast', 'Clients forecast')} loading={loadingPredictions} error={predictionError} onRetry={loadPredictions}>
                  {clientsPrediction ? (
              ...
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.sections.predictionStatus', 'Prediction status')} loading={loadingPredictions} error={predictionError} onRetry={loadPredictions}>
                  {predictionStatusRows.length || modelRuns.length ? (
                    <Space direction="vertical" size="middle" style={{ width: '100%' }}>
                      ...
                      {modelRuns.length ? <Divider style={{ margin: '8px 0' }} /> : null}
                      <PredictionModelStatus runs={modelRuns} loading={loadingPredictions} emptyText={tr('analyticsPage.empty.noPredictionStatus', 'No prediction status')} />
                    </Space>
                  ) : (
          ...
          >
            <Row gutter={[16, 16]}>
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.sections.recommendedActionsClients', 'Recommended actions (clients)')} loading={loadingPredictions} error={predictionError} onRetry={loadPredictions}>
              ...
              <Col xs={24} lg={12}>
                <AnalyticsCard title={tr('analyticsPage.sections.recommendedActionsDeals', 'Recommended actions (deals)')} loading={loadingPredictions} error={predictionError} onRetry={loadPredictions}>
    ...
    },
    {
      ...
      children: (
        ...
        >
          <AnalyticsCard title={tr('analyticsPage.sections.authStats', 'Auth stats')} loading={loadingAuthStats} error={authError} onRetry={loadAuthStats}>
            {authStats ? (
