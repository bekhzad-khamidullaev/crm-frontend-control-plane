...
/**
 * Export data to CSV
 * @param {string} entityType - Entity type (only `projects` supported by enterprise-crm-api.yaml)
...
export async function exportData(entityType, params = {}) {
  if (entityType !== 'projects') {
    throw new Error(
      `Export for "${entityType}" is not available in enterprise-crm-api.yaml. Only /api/projects/export/ is defined.`
    );
  }
...
/**
 * Export projects
 * Note: According to enterprise-crm-api.yaml, only /api/projects/export/ exists
 */
