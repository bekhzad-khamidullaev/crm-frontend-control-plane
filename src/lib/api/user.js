...
export async function getTwoFactorStatus() {
  try {
    ...
    console.warn('2FA endpoints not available');
 ...
 * Note: Requires backend implementation
 * @param {Object} data
 * @param {string} data.method - Method (totp, sms, email)
 * @returns {Promise<Object>}
 */
export async function enableTwoFactor(data) {
  try {
    return await api.post('/api/users/me/2fa/enable/', data);
  } catch (error) {
    console.warn('2FA enable endpoint not available');
    throw new Error('2FA functionality requires backend implementation');
  }
}
 ...
 */
export async function disableTwoFactor(data) {
  try {
    return await api.post('/api/users/me/2fa/disable/', data);
  } catch (error) {
    console.warn('2FA disable endpoint not available');
    throw new Error('2FA functionality requires backend implementation');
  }
}
...
export async function verifyTwoFactor(data) {
  try {
    return await api.post('/api/users/me/2fa/verify/', data);
  } catch (error) {
    console.warn('2FA verify endpoint not available');
    throw new Error('2FA functionality requires backend implementation');
  }
}
...
export async function getSecurityLog(params = {}) {
  try {
    return await api.get('/api/users/me/security-log/', { params });
  } catch (error) {
    console.warn('Security log not available');
    return { results: [], count: 0 };
  }
}
...
export async function revokeSession(sessionId) {
  try {
    ...
    console.warn('Session revoke not available');
    throw new Error('Session management requires backend implementation');
  }
}
