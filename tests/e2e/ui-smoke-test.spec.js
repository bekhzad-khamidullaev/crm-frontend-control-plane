...
test.describe('UI Smoke Tests - SMOKE_TEST_CHECKLIST', () => {
  ...
  const openModule = async (page, modulePath) => {
    ...
    await expect.poll(() => page.url(), { timeout: 10000 }).toMatch(buildModuleOrForbiddenRegex(modulePath));
  ...
  const modules = [
    { name: 'Дашборд коммуникаций', url: 'calls/dashboard' },
    { name: 'Чаты', url: 'chat' },
  ...
  const communicationRouteTabExpectations = {
    calls: /Omnichannel|Outbound \/ Broadcast/i,
    reminders: /Omnichannel|Outbound \/ Broadcast/i,
    'crm-emails': /CRM Email|Emails?/i,
    massmail: /Massmail|Рассыл|Ommaviy/i,
    sms: /Omnichannel|SMS/i,
  };
  ...
  for (const route of communicationRoutes) {
    test(`should open unified outbound composer from ${route}`, async ({ page }) => {
      ...
      if (hasActiveTab) {
        await expect(activeTab).toContainText(
          communicationRouteTabExpectations[route] || /Omnichannel|CRM Email|Massmail/i
        );
      } else {
        await expect(
          page.getByRole('heading', { name: /Outbound \/ Broadcast/i }).first()
        ).toBeVisible();
      }
      ...
      await expect(
        page
          .locator(
            'button:has-text("Отправить"), button:has-text("Send"), button:has-text("Создать"), button:has-text("Create")'
          )
          .first()
      ).toBeVisible();
    });
